...
= Developing cloud native microservices with the Eclipse MicroProfile Collection, Appsody CLI and Kabanero
...
Kabanero's Eclipse MicroProfile Collection provides a stack that enables the development and optimization of microservices. Applications are written based on the Eclipse MicroProfile API specifications, and built and run with the link:https://openliberty.io/[Open Liberty] runtime. And deployed with the Kabanero deploymeent model into your Application Develoopment cluster.
...
. *Optional*: You must have access to a local Kubernetes cluster for local developmeent use if you are testing multiple microservices together. If you are using Docker for Desktop, you can enable Kubernetes from the menu by selecting Preferences -> Kubernetes -> Enable Kubernetes.
. *Optional*: If you have a Kabanero instance that is used by your team for team based development, you must have access to this cluster provided to you by your administrator.
...
Add your Kabanero index to the Appsody CLI. The following example uses the public index for Kabanero Version 0.1.2. *Note*: Using the public Kabanero collection hub is for dimenstration only,  it is strongly recommended that you make your own private copy of the collection hub and you use it in the same way. But this demenstration does not require this.
...
*Recommendation*: The collections in the "kabanero collection hub", will be the only repoository that the application cluster will support.  This repository will have  the collections that the solution architect has curated for your business's use.  So its best to remove the "appsodyhub" from the list of supported taks.
----
...
Once you have built the application and done some form of unit test, which involves testing this container in isolation, it is desirable to test it as part of the system it will be part of. To do this you need to deploy the overall system, and then the new application that it is part of. You have some choices of how this is accomplished. You can deploy the application on a local Kubernetes running on your laptop, assuming that you have adequate CPU and Memory to run both mini-shift and the application and associated services that it uses.  Or, you can deploy the system, the changed application and associated services in a private name space on the server environment that has been set up on the development cluster.
